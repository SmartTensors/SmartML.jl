...
ScikitLearn.fit!(model, Matrix(x), Matrix(y_train))
...
ScikitLearn.fit!(model, Matrix(x), Matrix(y_train)[:,1])
xgb_model = model.best_estimator_
xgb_model.fit(Matrix(x), Matrix(y_train)[:,1])
...
def run_xgb_model(X_train, X_val, y_train, y_val):
...
PyCall.py"run_xgb_model"(Matrix(x), Matrix(x), Matrix(y_train)[:,1], Matrix(y_train)[:,1])