...
model_rf = ScikitLearn.GridSearch.RandomizedSearchCV(mod, param_dict; n_iter=10, cv=5, n_jobs=1)
ScikitLearn.fit!(model_rf, Matrix(x), Matrix(y))
rf_model = model_rf.best_estimator_
rf_model.fit(Matrix(x), Matrix(y))
...
model_xgb = ScikitLearn.GridSearch.RandomizedSearchCV(mod, param_dict; verbose=1, n_jobs=1, n_iter=10, cv=5)
ScikitLearn.fit!(model_xgb, Matrix(x), Matrix(y)[:,1])
...
y_pr, xgb_model = PyCall.py"run_xgb_model"(Matrix(x), Matrix(x), Matrix(y)[:,1], Matrix(y)[:,1])
NMFk.plotscatter(y_pr, Matrix(y)[:,1])